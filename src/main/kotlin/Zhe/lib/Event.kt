...
public class Event(val parser: Parser, val function: (values: Map<String, Token<*>>) -> Unit) : IEvent {
...
public class SequencingEvent : IEvent {
    ...
    private val _function: (values: Map<String, Token<*>>) -> Unit

    constructor(parsers: List<Parser>, function: (values: Map<String, Token<*>>) -> Unit) {
        ...
        this._parsers.mapIndexed{ index, parser -> 
            ...
            when{
                acc is Result.Fail || cur is Result.Fail -> Result.Fail("Not Match")
                    ...
                    val minRest: String = when(sAcc.rest.length < sCur.rest.length) { 
                        true -> sAcc.rest
                        else -> sCur.rest
                      }
